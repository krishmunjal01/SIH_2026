...
export default function DemoController() {
  ...
  return (
    <div className="absolute bottom-8 left-20 z-50 flex flex-col items-start space-y-2">
      ...
      <AnimatePresence>
        {currentStep > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 8 }}
            className="flex items-center space-x-2 bg-charcoal/90 backdrop-blur-md border border-accent/30 px-4 py-1.5 rounded-full text-xs font-mono text-accent shadow-[0_0_15px_rgba(59,130,246,0.2)]"
          >
            <motion.div
      ...
      {/* Button pill */}
      <div className="bg-charcoal/90 backdrop-blur-md border border-gray-700/50 rounded-full shadow-2xl p-2 flex items-center space-x-2">
        <button
          onClick={resetAll}
          className="px-4 py-2 text-xs font-semibold text-gray-300 hover:text-white hover:bg-gray-800 rounded-full transition-colors flex items-center space-x-1.5"
        >
          <RotateCcw size={13} />
          <span>Reset All</span>
        </button>
