...
export default function InfrastructurePanel() {
  ...
  return (
    <div className="absolute top-20 right-6 w-96 max-h-[80vh] flex flex-col bg-white/90 backdrop-blur-xl border border-gray-200/50 rounded-2xl shadow-2xl p-5 z-40 animate-in fade-in slide-in-from-right-10 duration-500">
      
      <div className="flex items-center space-x-3 mb-6">
        <RouteIcon className="text-accent" size={24} />
        <h2 className="text-gray-900 font-semibold tracking-wide text-lg">INFRASTRUCTURE HEALTH</h2>
      </div>

      <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        {segments.map((seg) => (
          <div key={seg.id} className="bg-slate-50/80 rounded-xl p-4 border border-gray-300">
              ...
              <div className="flex flex-col space-y-1">
                ...
                {seg.healthScore < 60 && (
                  <div className="text-[11px] font-mono mt-1 text-gray-500">
                    <span className="text-gray-500">ETA Impact:</span> Expected 10:45 AM -&gt; <span className="text-critical">Revised 10:53 AM</span>
                  </div>
                )}
              </div>
            )}
