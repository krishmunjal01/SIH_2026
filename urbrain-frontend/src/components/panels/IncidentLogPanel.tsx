...
export default function IncidentLogPanel() {
  ...
  return (
    <div className="absolute top-20 right-6 w-96 max-h-[80vh] flex flex-col bg-slate-50/95 backdrop-blur-xl border border-gray-200/50 rounded-2xl shadow-2xl p-5 z-40 animate-in fade-in slide-in-from-right-10 duration-500">
      
      <div className="flex items-center space-x-3 mb-6">
        <ShieldAlert className="text-critical" size={24} />
        <h2 className="text-gray-900 font-semibold tracking-wide text-lg">INCIDENT LOGS</h2>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
        {events.length === 0 ? (
          ...
          events.map((event) => (
            <div key={event.id} className={`bg-gray-100/80 rounded-xl p-3 border-l-4 border-gray-300 transition-colors cursor-pointer group ${event.status === 'VERIFIED' ? 'border-l-critical' : 'border-l-warning animate-pulse-slow'}`}>
              <div className="flex justify-between items-center mb-1">
                <span className="font-mono font-bold text-gray-800 text-sm">{event.id}</span>
              ...
              <div className="flex items-center space-x-2 text-xs text-gray-700 font-semibold mb-2">
